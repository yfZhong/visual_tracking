...
#include <visual_tracking/IcpPointMatcher/icpPointToPlane.h>
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r10,r11,t0,t1) // schedule (dynamic,2)
    for (i=0; i<nact; i++) {
      ...
      ///--------- by Yongfeng--------///
      double dist = (sx-dx)*nx+(sy-dy)*ny; 
      correspondences.push_back(T[idx*2+0]);
      correspondences.push_back(T[idx*2+1]);
      correspondences.push_back(sx);
      correspondences.push_back(sy);
      correspondences.push_back(dx);
      correspondences.push_back(dy);
      correspondences.push_back(sx - nx*dist);
      correspondences.push_back(sy - ny*dist);
      ///--------- by Yongfeng--------///
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) // schedule (dynamic,2)
	...
	///--------- by Yongfeng--------///
      double dist = (sx-dx)*nx+(sy-dy)*ny+(sz-dz)*nz;
      correspondences.push_back(T[idx*3+0]);
      correspondences.push_back(T[idx*3+1]);
      correspondences.push_back(T[idx*3+2]);
      correspondences.push_back(query[0]);
      correspondences.push_back(query[1]);
      correspondences.push_back(query[2]);
      correspondences.push_back(dx);
      correspondences.push_back(dy);
      correspondences.push_back(dz);
      correspondences.push_back(sx - nx*dist);
      correspondences.push_back(sy - ny*dist);
      correspondences.push_back(sz - nz*dist);
      inlierIdx.push_back(result[0].idx);
      ///--------- by Yongfeng--------///
...
std::vector<int32_t> IcpPointToPlane::getInliers (double *T,const int32_t T_num,const Matrix &R,const Matrix &t,const double indist) {
  ...
  if (dim==2) {
    ...
    for (int32_t i=0; i<T_num; i++) {
      ...
      //change by Yongfeng.  here is the old version, which I though forget t0 and t1
