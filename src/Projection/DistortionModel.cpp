...
// 		resCountour.push_back(Point(dv[1] , dv[0] ));
...
//-----------Yongfeng-----------//
  void DistortionModel::unDistortionModelInit(){
	...
	for (uint32_t i = 0; i < resCountourFloat.size(); i++)
	{
	    ...
	    if((int) round(resCountourFloat[i].x) >= 0 &&  (int) round(resCountourFloat[i].x) < W
	      && (int) round(resCountourFloat[i].y) >= 0 &&  (int) round(resCountourFloat[i].y) <H ) {
		 unDistortionModel.at<uchar>(p[i].y ,p[i].x) = 1;
	      }
	      
		
	}
// 	cv::imshow("unDistortionModel" ,unDistortionModel);
// 	cv::waitKey(10);
  }
//-----------Yongfeng-----------//
  ...
  double DistortionModel::Radian2Degree(double r)
  {
	  return (r / M_PI) * (180);
  }
  
  double DistortionModel::GetDistance(Point2d p)
  {
